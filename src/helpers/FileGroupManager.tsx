...
const fileGroupDefinitions: Record<string, Omit<FileGroupInfo, 'key'>> = {
    web: { label: 'Web & Markup', icon: <IconWorldWww size={18} />, color: 'orange' },
    data: { label: 'Data & DB', icon: <IconDatabase size={18} />, color: 'lime' },
    docs: { label: 'Docs & Prose', icon: <IconFileText size={18} />, color: 'gray' },
    config: { label: 'Build & Config', icon: <IconSettings size={18} />, color: 'grape' },
    shell: { label: 'Shell & OS Scripts', icon: <IconTerminal2 size={18} />, color: 'teal' },
    csharp: { label: 'C# / .NET', icon: <IconBrandCSharp size={18} />, color: 'violet' },
    c_cpp: { label: 'C / C++', icon: <IconBrandCpp size={18} />, color: 'blue' },
    jvm: { label: 'JVM Languages', icon: <IconFile size={18} />, color: 'red' },
    rust: { label: 'Rust', icon: <IconBrandRust size={18} />, color: 'orange' },
    python: { label: 'Python', icon: <IconBrandPython size={18} />, color: 'cyan' },
    go: { label: 'Go', icon: <IconBrandGolang size={18} />, color: 'indigo' },
    functional: { label: 'Functional', icon: <IconMathFunction size={18} />, color: 'pink' },
    scripting: { label: 'General Scripting', icon: <IconFileCode size={18} />, color: 'yellow' },
    programming: { label: 'Other Languages', icon: <IconCode size={18} />, color: 'blue' },
    images: { label: 'Images', icon: <IconPhoto size={18} />, color: 'green' },
    other: { label: 'Other Files', icon: <IconFile size={18} />, color: 'gray' },
};

const fileGroups: Record<string, FileGroupInfo> = Object.fromEntries(
    Object.entries(fileGroupDefinitions).map(([key, value]) => [key, { ...value, key }])
);

const extensionToGroupKeyMap = new Map<string, string>();
...
const typeToGroupKeyMap: Record<string, string> = {
    markup: 'web',
    data: 'data',
    prose: 'docs',
...
const languageData: FileTypeDefinition[] = [
    ...
    { "name": "GLSL", "type": "programming", "extensions": [".glsl", ".fp", ".frag", ".frg", ".fs", ".fsh", ".fshader", ".geo", ".geom", ".glslv", ".gshader", ".shader", ".vert", ".vrx", ".vsh", ".vshader"] },
    ...
    { "name": "Groff", "type": "markup", "extensions": [".man", ".1", ".1in", ".1m", ".1x", ".2", ".3", ".3in", ".3m", ".3qt", ".3x", ".4", ".5", ".6", ".7", ".8", ".9", ".l", ".me", ".ms", ".n", ".rno", ".roff"] },
    ...
    { "name": "Ruby", "type": "programming", "extensions": [".rb", ".builder", ".fcgi", ".gemspec", ".god", ".irbrc", ".jbuilder", ".mspec", ".pluginspec", ".podspec", ".rabl", ".rake", ".rbuild", ".rbw", ".rbx", ".ru", ".ruby", ".thor", ".watchr"] },
...
languageData.forEach(lang => {
    ...
    lang.extensions.forEach(ext => {
        ...
        if (!extensionToGroupKeyMap.has(cleanExt)) {
            extensionToGroupKeyMap.set(cleanExt, groupKey);
        }
    });
});

const imageExtensions = ['png', 'jpg', 'jpeg', 'gif', 'svg', 'webp', 'ico'];
imageExtensions.forEach(ext => extensionToGroupKeyMap.set(ext, 'images'));

const specialFileMap: Record<string, { icon: ReactNode; color: string; groupKey: string }> = {
    '.gitignore': { icon: <IconBrandGit size={18} />, color: 'orange', groupKey: 'config' },
    'vite.config': { icon: <IconBrandVite size={18} />, color: 'grape', groupKey: 'config' },
    '.env': { icon: <IconSettings size={18} />, color: 'grape', groupKey: 'config' },
};

export const getGroupInfoForFile = (fileName: string): FileGroupInfo => {
    for (const specialName in specialFileMap) {
        if (fileName.includes(specialName)) {
            return { ...fileGroups[specialFileMap[specialName].groupKey], ...specialFileMap[specialName] };
        }
    }
